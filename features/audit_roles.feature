Feature: audit with additional roles
  ...
  Scenario: with one additional role
    ...
    """
    $conjur.with_audit_roles('cucumber:user:auditor1').resource('variable:$ns_foo').permitted?('read')
    """
  ...
  Scenario: with more than one additional role
    ...
    When I evaluate the expression 
    """
    $conjur.with_audit_roles(['cucumber:user:auditor2','cucumber:group:auditors'])
